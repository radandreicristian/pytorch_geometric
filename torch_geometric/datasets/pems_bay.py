...
from torch_geometric.io.pems_bay import PemsBayIo
from torch_geometric.utils.sparse import dense_to_sparse


class PemsBay(Dataset):
    r"""The Performance Measurement System - Bay Area (Pems-Bay) highway traffic
    dataset introducted in  the `"Big Data and its Technical Challenges"
    <http://citeseerx.ist.psu.edu/viewdoc/download
    ?doi=10.1.1.681.7248&rep=rep1&type=pdf>`_
    paper.

    Nodes represent sensors in a sensor network, each being able to
    detect cars passing by it. The data points represent the average
    traffic speed, aggregated in 5 minutes
    ...
    def __init__(self, root: Optional[str], n_previous_steps: int,
        ...
        self.name = 'PeMS-Bay-OnDisk'
        self.io = PemsBayIo(n_readings=self.n_readings,
    ...
    def process(self) -> None:
        ...
        x, y = self.io.get_pems_data(data_path=self.raw_paths[2])
    ...
    def len(self) -> int:
        r"""Return the number of graphs stored in the dataset."""
    ...
    def get_adjacency_matrix(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Return the graph's adjacency matrix in sparse COO format.
...
class PemsBayInMemory(InMemoryDataset):
    ...
    def download(self) -> None:
        r"""Download the dataset to the :obj:`self.raw_dir` folder."""
        for file_name, gdrive_id in self.gdrive_ids.items():
            gdd.download_file_from_google_drive(
                file_id=gdrive_id, dest_path=osp.join(self.raw_dir, file_name))
